...
def train(args, train_dataset, model, tokenizer):
    ...
    logger.info("  Total Model parameters - %d", sum(p.numel() for p in model.parameters()))
    logger.info("  Trainable Model parameters - %d", sum(p.numel() for p in model.parameters() if p.requires_grad))
...
def quantize_init(model, scheme="range-based-log", bits=3):
    ...
    if scheme=="no-quantize":
        ...
        vals = torch.FloatTensor([2**(base*i + offset_val + min_exp) for i in range(int(2**bits)-1)])
        for layer_idx in range(len(model.roberta.encoder.layer)):
            model.roberta.encoder.layer[layer_idx].attention.self.quantizer.init_weights(weights, vals)
        ...
        vals = torch.FloatTensor([2**(base*i + offset_val + min_exp) for i in range(int(2**bits)-1)])
        for layer_idx in range(len(model.roberta.encoder.layer)):
            model.roberta.encoder.layer[layer_idx].attention.self.quantizer.init_bounds(bounds=bounds, vals=vals)
...
def set_quantize(layers, model, quantize=True):
    ...
    #for layer_idx in range(13):
        #layer = model.roberta.embeddings.parameters() if layer_idx == 0 else model.roberta.encoder.layer[layer_idx-1].parameters()
        #for params in layer:
